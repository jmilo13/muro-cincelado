...
export const renderBlock = (block, nextItem) => {
  ...
  switch (type) {
    ...
    case "numbered_list_item":
      if (nextItem) {
        const itemList = []
        itemList.push({children: children, has_children: has_children})
        value.rich_text.map(item => itemList.push({text: item.plain_text, url: item.href})) 
        list.push(itemList)
        break
      }else {
        const itemList = []
        itemList.push({children: children, has_children: has_children})
        value.rich_text.map(item => itemList.push({text: item.plain_text, url: item.href, children: children}))
        list.push(itemList)
        ...
        if(type === "bulleted_list_item") {
          ...
          return (
            <ul key={id}>
              {listComplete.map(item => {
                ...
                return (
                <>
                <li key={item.id}>{item.map(subItem => subItem.url?<a href={subItem.url} target="_blank">{subItem.text}</a> : subItem.text)}</li>
                { item[0].has_children &&
          ...
          return (
            <ol key={id}>
              {listComplete.map(item => <li key={item.id}>{item}</li>)}
              {has_children && children.map(item => <p className="embebed">{item[type].rich_text[0].plain_text}</p>)}
              <style jsx>
              {`
                .embebed {
                  padding-left: 25px;
    ...
    case "quote":
      return (
      <blockquote key={id}>{value.rich_text[0].plain_text}
        <style jsx>
          {`
          blockquote {
            margin: 16px 0;
